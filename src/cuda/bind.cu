...
std::tuple<torch::Tensor, torch::Tensor> knn(torch::Tensor xi_, torch::Tensor xf_, int k)
{
    ...
    torch::Tensor ws_ = torch::zeros({batch_size, k}, torch::dtype(torch::kFloat32).device(torch::kCUDA));
    PitchedPtr<long, 3> neigs((long *)neigs_.data_ptr(), batch_size, k, n_dim);
    PitchedPtr<float, 2> ws((float *)ws_.data_ptr(), batch_size, k);
    ...
    parallel_for(batch_size, [=] __device__(int i) {
        ...
        for (int j = 0; j < k; j++)
        {
            for (int d = 0; d < n_dim; d++)
            {
                ...
                {
                    neigs(i, j, d) = x_int + 1;
                    ws_tmp[j] += (1 - x_intf) * (1 - x_intf);
                }
                else
                {
                    neigs(i, j, d) = x_int;
                    ws_tmp[j] += x_intf * x_intf;
                }
            }
            ws_tmp[j] = sqrt(ws_tmp[j]);
            ws_sum += ws_tmp[j];
        }
        for (int j = 0; j < k; j++)
        {
            ws(i, j) = ws_tmp[j] / ws_sum;
        }
    });

    return std::make_tuple(neigs_, ws_);
}
